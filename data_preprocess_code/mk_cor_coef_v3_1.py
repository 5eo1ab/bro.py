...
org_var_list = ['_Close', '_Volume', '_Beta_1', '_Beta_3',  '_Beta_6', '_Beta_12']
var_list = ['_Close', '_Volume', '_Beta_12']
...
index_nm_list = ['NIKKEI225', 'SnP500', 'SSE50', 'DAX30', 'KOSPI200']
for indexnm in temp:
    skip_corp_list = []
    ...
    if indexnm[-4:] == '.csv': #csv 파일 아니면 continue
        ...
        corp_list =[data.columns[i][:-6] for i in range(len(data.columns)) if i % len(org_var_list )==0 ]
        ...
        for i in range(len(corp_list)):
            print(corp_list[i])
            for j in range(i+1, len(corp_list)):
                ...
                intersection_date = set(X.index[list(~np.isnan(X.loc[:,corp1+var_list[-1]]))]).intersection(Y.index[list(~np.isnan(Y.loc[:,corp2+var_list[-1]]))])
        ...
        if not os.path.exists(directory+"\\RV_coeff"+read_cvb_directory[-5:-2]+ "v4"):
                os.makedirs(directory+"\\RV_coeff"+read_cvb_directory[-5:-2]+ "v4")
                
                
        skip_corp_txt = ''
        skip_corp_list = list(set(skip_corp_list))
              
        rv_coef_df.to_csv(directory+"\\RV_coeff"+read_cvb_directory[-5:-2]+ "v4\\RV_Coeff_" + indexnm[18:])
