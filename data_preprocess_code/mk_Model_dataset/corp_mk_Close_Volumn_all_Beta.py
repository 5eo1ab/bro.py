...
mk_directory = '\\Preprocessed_dataset\\'+ price+'_Volume_all_Beta'
rd_directory ='Raw_DataSet\\'
...
for indexnm in index_list:
    ...
    for compnm in os.listdir(rd_directory +indexnm):
        print('--------------------',compnm,'-----------------------')
        
        #데이터 읽어오기(파일 이름에 문제있어서 안 읽어 지는 부분 예외처리)
        try:
            data_org = pd.read_csv(rd_directory + indexnm+'\\'+compnm)
            ...
            try:#파일이름 변환 
                os.rename(rd_directory + indexnm+'\\'+compnm, rd_directory +indexnm+'\\' + mod_compnm)
                data_org = pd.read_csv(rd_directory +indexnm+'\\' + mod_compnm)
        ...
        while True:
            ...
            if cursor <=0:
                break        
            time_beta_dict[str(sorted_data.iloc[cursor]['Date'])[0:7]] =  close_volume_beta #월 단위까지만 본다
            cursor, end_day = end_date_index(sorted_data,cursor) 
               
                      
                    
        if time_beta_dict == {}:
            err_msg +=  "beta값이 비었음 : " + indexnm + ', ' + compnm + '\n'
            continue
