...
def get_diff_df(df_in) :
    from pandas import DataFrame as df
    #df_in =dic_n_t_df[cntry_nm[i]]
    diff_df = []
    #arr_tl = df_in['TimeLog'][1:]
    #df_in = df_in[list(df_in.columns.values)[1:]]
    df_idx = df_in.index[1:]

    for i in range(len(df_idx)):
        row = (np.array(df_in[df_in.index == df_idx[i]])[0] - np.array(df_in[df_in.index == df_idx[i-1]])[0])[0]
        row = (row/(np.array(df_in[df_in.index== df_idx[i-1]]))*100)[0]
        row = [0 if np.isinf(row)[j] or np.isnan(row)[j]   else row[j] for j in range(len(row))]        
        diff_df.append(row)
    df_out = df(diff_df, columns= list(df_in.columns.values))
    df_out.index = df_idx
    return df_out
...
#################################################################
os.chdir(directory)
rd_inpt_dir = 'Preprocessed_dataset\\Modeling_Data_all\\'
rd_tgt_dir = 'dataset_competition\\'
...
input_files =['preprocess_nan_DAX30_all.csv', 'preprocess_nan_kospi200_all_all.csv', 'preprocess_nan_nikkei225_all.csv', 'preprocess_nan_SSE50_all.csv', 'preprocess_nan_SnP500_all.csv']



target_data = pd.read_csv(rd_tgt_dir  +'IMPORTED_CSV\\G_IDX_CLOSE.csv')
target_data_cls = pd.read_csv(rd_tgt_dir +'Y_IDX_CLOSE.csv')
...
for i in range(len(input_files)):
    ...
    input_data = pd.read_csv(rd_inpt_dir+input_files[i])         
    ...
    temp_in_df = get_modeling_input(input_df, lag_t)
    temp_in_df.index = temp_in_df['TimeLog']
    temp_in_df = temp_in_df.drop('TimeLog', 1)
    
    dic_n_t_df[cntry_nm[i]] =temp_in_df
...
for i in range(len(input_files)):
    ...
    dic_n_corr_df[cntry_nm[i]] =corr_li
...
for i in range(len(input_files)):
    ...
    X = dic_n_diff_df[cntry_nm[i]][:-48]
    y = dic_n_tgt_cls[cntry_nm[i]][1:-48].values.ravel()
...
sltd_dir = 'Sltd_in_var_data\\'

if not os.path.exists(rd_inpt_dir+sltd_dir +'result_report'):
    os.makedirs(rd_inpt_dir+sltd_dir + 'result_report')
var_counts.to_csv(rd_inpt_dir+sltd_dir + 'result_report\\'+"var_counts.csv", index=False)





if not os.path.exists(rd_inpt_dir + sltd_dir + 'Input_CSV\\'):
        os.makedirs(rd_inpt_dir + sltd_dir + 'Input_CSV\\')

for i in range(len(input_files)):
    dic_n_diff_df[cntry_nm[i]][dic_union[cntry_nm[i]]].to_csv(rd_inpt_dir + sltd_dir + 'Input_CSV\\union_{}_total.csv'.format(cntry_nm[i]), index='TimeLog')
    dic_n_diff_df[cntry_nm[i]][dic_union[cntry_nm[i]]][:-48].to_csv(rd_inpt_dir + sltd_dir + 'Input_CSV\\union_{}_train.csv'.format(cntry_nm[i]), index='TimeLog')
    dic_n_diff_df[cntry_nm[i]][dic_union[cntry_nm[i]]][-48:].to_csv(rd_inpt_dir + sltd_dir + 'Input_CSV\\union_{}_test.csv'.format(cntry_nm[i]), index='TimeLog')




for i in range(len(input_files)):
    dic_n_diff_df[cntry_nm[i]][dic_intersection[cntry_nm[i]]].to_csv(rd_inpt_dir + sltd_dir + 'Input_CSV\\intersection_{}_total.csv'.format(cntry_nm[i]), index='TimeLog')
    dic_n_diff_df[cntry_nm[i]][dic_intersection[cntry_nm[i]]][:-48].to_csv(rd_inpt_dir + sltd_dir + 'Input_CSV\\intersection_{}_train.csv'.format(cntry_nm[i]), index='TimeLog')
    dic_n_diff_df[cntry_nm[i]][dic_intersection[cntry_nm[i]]][-48:].to_csv(rd_inpt_dir + sltd_dir + 'Input_CSV\\intersection_{}_test.csv'.format(cntry_nm[i]), index='TimeLog')
...
merged_df.to_csv(rd_inpt_dir + sltd_dir +"Y_bin_total.csv", index='TimeLog')
merged_df[:-48].to_csv(rd_inpt_dir + sltd_dir + ''+"Y_bin_train.csv", index='TimeLog')
merged_df[-48:].to_csv(rd_inpt_dir + sltd_dir + ''+"Y_bin_test.csv", index='TimeLog')
