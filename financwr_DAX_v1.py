...
def mk_comp_code_dict(file_nm):
    comp_code_dict = dict()
    list_file =pd.read_excel('DAX30_data\\'+file_nm)
...
comp_code_dict = mk_comp_code_dict('DAX30_wiki.xlsx')    
...
for key in comp_code_dict.keys():
    try:
        ...
        new =  r'C:\Users\yeohyeongyu\Desktop\finance_data\DAX30\\'+str(comp_code_dict[key])+'_'+str(key)+'.csv'
...
err_text += '-----------------------------------------------------\n\n'
print(err_text)
with open('DAX30\\err_text.txt', "w") as err:
    err.write(err_text)
