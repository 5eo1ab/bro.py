...
for indexnm in index_file:
    ...
    for period in period_list:
        ...
        data_org= data_org[data_org.index <= "2016-12"]
        
        
        beta_cols= [ beta for beta in data_org.columns if beta[-4:] =='Beta']
        
        if count == 0:
            data = data_org.copy()
            data = data.drop(beta_cols, axis =1)
        ...
        for beta in beta_cols:
            data[beta + '_'+str(period)]   = data_org.loc[:,beta]
    ...
    print(indexnm, 3)
    if not os.path.exists(directory+"\\Modeling_RawData_all"):
            os.makedirs(directory+"\\Modeling_RawData_all")
    
    rm_data.to_csv(directory+"\\Modeling_RawData_all\\Modeling_dataset_" + indexnm[18:-4] + '_all.csv')
